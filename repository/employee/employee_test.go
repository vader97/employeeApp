...
func TestInMemoryEmployeeRepository(t *testing.T) {
	...
	retrievedEmp, _ = repo.GetEmployeeByID(1)
	if retrievedEmp.Salary != 60000 {
		t.Errorf("Employee update failed")
	}

	// Test DeleteEmployee
	if err := repo.DeleteEmployee(1); err != nil {
	...
	employees, err := repo.ListEmployees(1, 10)
	if err != nil {
